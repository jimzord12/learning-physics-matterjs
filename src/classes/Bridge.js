(function () {
  const { Bodies, Composite, Composites } = Matter;
  ...
  class Bridge {
    ...
    changeFirstBallPosition(x, y) {
      this.balls[0].body.position.x = x;
      this.balls[0].body.position.y = y;
    }

    /**
     * Resets the position of the last ball to the initial state.
     * @param {number} x - The x position of the last ball.
     * @param {number} y - The y position of the last ball.
     */
    changeLastBallPosition(x, y) {
      this.balls[this.balls.length - 1].body.position.x = x;
      this.balls[this.balls.length - 1].body.position.y = y;
    }
  }
